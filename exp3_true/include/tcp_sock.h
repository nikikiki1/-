...
#define TCP_MAX_BACKLOG 128
	...
	struct tcp_timer persist_timer;   // 指向persist timer的指针
...
//添加了一些新的变量
	int new_ack;
	int c_state;// 当前状态
	int ack_count;     // 重复ACK计数
	u32 last_ack;        // 上次收到的ACK序列号
...
// 接收乱序缓冲区条目，存储接收到但不按顺序的数据包
struct recv_ofo_buf_entry {
    struct list_head list;    // 链表节点
    char *packet;               // 数据包内容（仅TCP数据部分）
    int len;                  // 数据长度
    u32 seq;                  // 起始序列号
    u32 seq_end;              // 结束序列号（seq + len）
};
